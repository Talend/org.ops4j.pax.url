...
// The job packages osp4j_2.6.2 with Talend's patch
// https://github.com/Talend/org.ops4j.pax.url/tree/url-2.6.2-tipaas
    ...
    stages {
          ...
          steps {
            container('maven') {
              configFileProvider([configFile(fileId: 'maven-settings-nexus-zl', variable: 'MAVEN_SETTINGS')]) {
                ...
                archiveArtifacts artifacts: 'pax-url-aether/target/pax-url-aether-2.6.2-*.jar', fingerprint: true, onlyIfSuccessful: true
              }
            }
          }
        }
        ...
        stage('Publlish to Nexus') {
          ...
          steps {
            container('maven') {
              configFileProvider([configFile(fileId: 'maven-settings-nexus-zl', variable: 'MAVEN_SETTINGS')]) {
                sh "mvn deploy:deploy-file -s $MAVEN_SETTINGS -DgeneratePom=true -DrepositoryId=thirdparty-releases -DgroupId=org.ops4j.pax.url -DartifactId=pax-url-aether -Dversion=2.6.2 -Dclassifier=${params.CLASSIFIER} -Dpackaging=jar -Durl=https://artifacts-zl.talend.com/nexus/content/repositories/thirdparty-releases -Dfile=pax-url-aether/target/pax-url-aether-2.6.2-${params.CLASSIFIER}.jar"
              }
            }
          }
        }
